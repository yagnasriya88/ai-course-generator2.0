...
function DiagramJobNotificationToast() {
  const { notice, clearNotice } = useDiagramJobs()
        ...
        >
          ...
          <div>
            <p className="text-sm font-semibold text-slate-900">
              {succeeded ? 'Your diagram is ready!' : 'Diagram generation failed'}
            </p>
          ...
          {succeeded && (
            <button
              type="button"
              onClick={() => {
                navigate(`/diagram/${notice.diagram_id}`)
                clearNotice()
              }}
...
export default DiagramJobNotificationToast
